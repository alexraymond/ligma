...
winget_root="packaging/winget/manifests/l/alexraymond/Ligma"
...
if [[ -d "$winget_dir" ]]; then
  ...
  installer="$winget_dir/alexraymond.Ligma.installer.yaml"
...
new_block = (
    "Installers:\n"
    f"  - Architecture: x64\n"
    f"    InstallerUrl: https://github.com/alexraymond/ligma/releases/download/v{version}/ligma-{version}-x64-setup.exe\n"
    f"    InstallerSha256: {x64.upper()}\n"
    f"  - Architecture: arm64\n"
    f"    InstallerUrl: https://github.com/alexraymond/ligma/releases/download/v{version}/ligma-{version}-arm64-setup.exe\n"
    f"    InstallerSha256: {arm64.upper()}\n"
)
out = re.sub(r"Installers:\n(?:(?:  -|    ).*\n)+", new_block, src, count=1)
open(path, "w").write(out)
PY
  locale="$winget_dir/alexraymond.Ligma.locale.en-US.yaml"
