...
│   └── Casks/ligma.rb
├── winget/
│   └── manifests/l/alexraymond/Ligma/<version>/
│       ├── alexraymond.Ligma.yaml
│       ├── alexraymond.Ligma.installer.yaml
│       └── alexraymond.Ligma.locale.en-US.yaml
├── scoop/
│   └── bucket/ligma.json
├── flatpak/
│   └── com.ligma.app.yaml
...
### Homebrew Cask — `alexraymond/homebrew-tap`
...
gh repo create alexraymond/homebrew-tap --public \
  --description "Homebrew tap for Ligma and friends"
git clone git@github.com:alexraymond/homebrew-tap.git /tmp/homebrew-tap
...
```sh
brew tap alexraymond/tap
brew install --cask ligma
```

### winget — `microsoft/winget-pkgs`

Microsoft's monorepo. Fork it, copy `packaging/winget/manifests/l/alexraymond/Ligma/<version>/` into the same path in the fork, open a PR. `wingetcreate validate` is worth running first:

```sh
wingetcreate validate packaging/winget/manifests/l/alexraymond/Ligma/0.1.2
```
...
### Scoop — `alexraymond/scoop-bucket`

Separate public bucket repo. Copy `packaging/scoop/bucket/ligma.json` to its `bucket/` directory.

```sh
gh repo create alexraymond/scoop-bucket --public \
  --description "Scoop bucket for Ligma"
git clone git@github.com:alexraymond/scoop-bucket.git /tmp/scoop-bucket
...
```pwsh
scoop bucket add ligma https://github.com/alexraymond/scoop-bucket
