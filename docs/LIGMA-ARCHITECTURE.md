...
## Workstream map (v0.1.0)

- **W1 — Reliability.** Pre-warmed CLI path, IPC-ACK contract, stream truncation + heartbeat, unknown-path warnings, `allowedTools` hand-off seam.
- **W2 — Agent loop.** Async-generator `runTurn`, typed `AgentEvent` union, concurrency-safe tool orchestration, fs-read / fs-write tools, SDK → agent-event adapter.
- **W3 — Rebrand.** `@ligma/*` workspaces, `com.ligma.app` appId, `~/.config/ligma/` storage paths, README / CHANGELOG / CITATION / CONTRIBUTING rewrites.
- **W4 — Session log.** Append-only JSONL + content-addressed file versioning, cursor pagination, resume-with-truncation-recovery, session IPC handlers.
- **W5 — Overnight infrastructure.** Worktree setup, merge, and report scripts; swarm PRD; workstream state directory.
- **W6 — Docs.** `NOTICE.md`, `docs/LIGMA-ARCHITECTURE.md`, `docs/RELIABILITY-AUDIT-2026-04.md`, final `CHANGELOG.md` and `README.md` polish.
- **W7 — UI reskin.** Dark-theme default, placeholder Ligma accent, two-circle icon, window chrome, tightened spacing, launch-smoke + token-drift tests.

## Why these choices?

The architectural patterns were studied directly from the Claude Code source tree on disk at `/Users/alexraymond/collection-claude-code-source-code/claude-code-source-code/src/`. The code in this repo is not copied from there — these are patterns we evaluated and then wrote from scratch for our use case.

- Async-generator agent loop — `src/query.ts:219-250`.
- Concurrency-safe tool partitioning — `src/services/tools/toolOrchestration.ts:8-189`.
- Append-only session log with sibling entry types — `src/assistant/sessionHistory.ts:1-88`.
- Typed event union (`TextChunk | ThinkingChunk | ToolStart | ToolEnd | PermissionRequest | TurnDone`) — `src/entrypoints/sdk/coreTypes.ts:25-53`.

Each pattern was chosen because it cleanly separates concerns that otherwise tangle: UI, session log, and permission gating subscribe to the same event stream without stepping on each other; read-only tools fan out without stalling behind a single slow write; the loop stays a generator so abort, error, and completion are all the same kind of control-flow event. If future maintenance needs to deviate, these are the reference points to re-read before replacing.
