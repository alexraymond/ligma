...
> **Scope note:** this describes the **legacy Electron desktop app** only —
> chat UI, main-process agent loop, IPC to a spawned Claude CLI. That app is
> **no longer in this repo**: it was removed from the tree and lives on only
> in the separate `ligma-classic` repo. This file is kept here as its frozen
> architecture record; every path it names is a path in `ligma-classic`, not
> in this checkout. It is not the current product architecture: the daemon +
> web + CLI factory (the thing `README.md` and `docs/ligma-build-brief.md`
> describe) is a separate, unrelated pipeline with no coupling to this one.
> See [`docs/architecture.md`](../architecture.md) for that current-state
> picture.
