...
export function replaceEditmodeBlock(source: string, newTokens: Record<string, unknown>): string {
  ...
  if (EDITMODE_RE.test(source)) {
    // Replacer function, not a template string: token values are model output
    // and `$&` / `$1` / `$'` inside one would be re-expanded as capture-group
    // patterns by String.replace, silently corrupting the block.
...
export function replaceTweakSchema(source: string, schema: TweakSchema): string {
  ...
  if (TWEAK_SCHEMA_RE.test(source)) {
    // See replaceEditmodeBlock — replacer function keeps `$`-patterns literal.
    return source.replace(
      TWEAK_SCHEMA_RE,
      () => `/*TWEAK-SCHEMA-BEGIN*/${json}/*TWEAK-SCHEMA-END*/`,
    );
  }
