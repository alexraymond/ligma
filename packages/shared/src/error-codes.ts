/**
 * Central registry of LigmaError codes used throughout the app.
 *
...
export const ERROR_CODES = {
  ...
  EXPORTER_IMAGE_FAILED: 'EXPORTER_IMAGE_FAILED',
...
/** Literal union of every known LigmaError code. */
export type LigmaErrorCode = (typeof ERROR_CODES)[keyof typeof ERROR_CODES];

/** @deprecated renamed to `LigmaErrorCode` — kept for external consumers (desktop) mid-rename. */
export type CodesignErrorCode = LigmaErrorCode;
...
export const ERROR_CODE_DESCRIPTIONS: Record<LigmaErrorCode, ErrorCodeDescription> = {
  // IPC validation
  IPC_BAD_INPUT: {
  ...
  EXPORTER_IMAGE_FAILED: {
    userFacing: 'Image export failed. Ensure Chrome is installed and try again.',
    userFacingKey: 'err.EXPORTER_IMAGE_FAILED',
    category: 'other',
  },
