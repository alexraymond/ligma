/**
 * @ligma/shared — the types every other package agrees on.
 *
 * Zod schemas and TypeScript types for projects, artifacts, chat messages,
 * providers and config; the `LigmaError` + `ERROR_CODES` taxonomy; and a
 * handful of pure helpers (config parsing/migration, EDITMODE marker
 * rewriting, fingerprints, base-url detection). No I/O, no provider SDKs —
 * this package sits at the bottom of the dependency graph
 * (shared <- providers <- core).
 */
import { z } from 'zod';
import type { LigmaErrorCode } from './error-codes';
...
export class LigmaError extends Error {
  /**
   * Upstream HTTP status, when the boundary that raised this error knew one.
   * Carried structurally so downstream classifiers (retry policy, provider
   * error remapping) never have to scrape a status out of the message prose —
   * a "exceeded 512 tokens" message used to classify as a retryable 5xx.
   */
  ...
  constructor(
    ...
    public readonly code: LigmaErrorCode | string,
    options?: { cause?: unknown; status?: number },
  ) {
    super(message, options);
    this.name = 'LigmaError';
    if (options?.status !== undefined) this.status = options.status;
  }
}

/** @deprecated renamed to `LigmaError` — kept for external consumers (desktop) mid-rename. */
export { LigmaError as CodesignError };
...
export type { LigmaErrorCode } from './error-codes';
/** @deprecated renamed to `LigmaErrorCode` — kept for external consumers (desktop) mid-rename. */
...
export interface ReportableError {
  ...
  localId: string;
  /** LigmaError code / err.name / 'RENDERER_ERROR' default. */
  code: string;
 ...
 *
 * When unset, the agent inherits the main process cwd — wherever the host
 * was launched from, which is rarely what users want to scan against.
 */
