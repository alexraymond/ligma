...
/**
 * System-managed provider id for Claude Max subscription via the local
 * Claude Code CLI (@anthropic-ai/claude-agent-sdk picks up the logged-in
 * session from Keychain/filesystem). Fork-local: Anthropic's ToS restricts
 * claude.ai-auth-driven third-party products. Same shape precedent as
 * CHATGPT_CODEX_PROVIDER_ID above.
 */
