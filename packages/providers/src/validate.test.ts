...
describe('pingProvider', () => {
  it('throws LigmaError for unsupported providers', async () => {
    await expect(pingProvider('google', 'AIza-foo')).rejects.toBeInstanceOf(LigmaError);
  });

  it('throws when key is empty', async () => {
    await expect(pingProvider('anthropic', '')).rejects.toBeInstanceOf(LigmaError);
  });
