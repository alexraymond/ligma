...
/**
 * Look `claude` up on the PATH this process inherited.
 *
 * `which` does not exist on Windows; `where.exe` does, and prints one match
 * per line. (P12 — the daemon's copy of this resolution already branched on
 * platform and this one did not.)
 */
function lookupOnProcessPath(): string | null {
  const [cmd, args] =
    process.platform === 'win32'
      ? (['where', ['claude']] as const)
...
/**
 * Second chance for GUI launches (S4 / D5). A Finder- or Dock-launched app
 * inherits launchd's minimal PATH — `/usr/bin:/bin:/usr/sbin:/sbin` — which
 * contains neither the npm global prefix nor Homebrew, so `which claude` finds
 * nothing even though the user's terminal resolves it fine. Ask an interactive
 * login shell (the one that sources the user's profile) to do the lookup.
 *
 * Skipped on Windows, which has no login-shell notion and whose GUI processes
 * inherit the full user PATH already.
 */
function lookupViaLoginShell(): string | null {
  if (process.platform === 'win32') return null;
  const shell = process.env['SHELL'];
  if (!shell || shell.length === 0) return null;
  try {
    // -l sources the login profile; `command -v` is POSIX and avoids shell
    // builtins that print more than the path. The argv is static — the only
    // interpolation-free way to run this.
    return firstLine(
      execFileSync(shell, ['-lic', 'command -v claude'], {
        encoding: 'utf8',
...
/**
 * Resolve the `claude` binary and memoise the result for the process lifetime.
 * Idempotent: safe to call from boot-init or test setup. Returns `null` when
 * the CLI cannot be found by either strategy.
 *
 * This is the ONLY function that should ever shell out to resolve `claude`.
...
export function prewarmClaudeExecutable(): string | null {
  ...
  cachedClaudePath = lookupOnProcessPath() ?? lookupViaLoginShell();
...
export interface ClaudeCliCompleteOptions {
   ...
   * inherits the host process cwd — wherever the host happens to have been
   * launched from, which is rarely what users expect.
   */
  cwd?: string;
  /**
