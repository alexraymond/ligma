...
describe('prewarmClaudeExecutable', () => {
  ...
  // D5 / seam S4 — a Finder-launched build inherits launchd's minimal PATH, so
  // `which` misses npm-global / Homebrew installs. Ask a login shell instead
  // before declaring the whole Claude Max provider unusable.
  it('falls back to a login-shell lookup when the inherited PATH misses it', () => {
    vi.stubEnv('SHELL', '/bin/zsh');
    vi.mocked(execFileSync).mockImplementationOnce(() => {
      throw new Error('not found');
    });
    vi.mocked(execFileSync).mockImplementationOnce(() => '/opt/homebrew/bin/claude\n');

    expect(prewarmClaudeExecutable()).toBe('/opt/homebrew/bin/claude');
  ...
  it('returns null when neither PATH nor the login shell resolves it', () => {
    vi.stubEnv('SHELL', '/bin/zsh');
  ...
  // P12 — `which` does not exist on Windows; the daemon's twin already knew.
  it('uses `where` on win32', () => {
    const platform = process.platform;
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      prewarmClaudeExecutable();
      expect(execFileSync).toHaveBeenCalledWith('where', ['claude'], expect.any(Object));
    } finally {
