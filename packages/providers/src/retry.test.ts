import { type ChatMessage, LigmaError, type ModelRef } from '@ligma/shared';
...
describe('classifyError', () => {
  ...
  // P8 — a `Retry-After: 3600` used to stall generation for an hour.
  it('clamps an absurd retry-after to the ceiling', () => {
    const d = classifyError(new HttpError('slow', 429, { 'retry-after': '3600' }));
    expect(d.retryAfterMs).toBe(60_000);
  });
  it('clamps an absurd HTTP-date retry-after to the ceiling', () => {
    const far = new Date(Date.now() + 3_600_000).toUTCString();
    const d = classifyError(new HttpError('slow', 429, { 'retry-after': far }));
    expect(d.retryAfterMs).toBe(60_000);
  });

  // P8 — the status used to be scraped as "first 3-digit number in the
  // message", so a token-count in the prose classified as a retryable 5xx.
  it('does not read a status out of message prose', () => {
    const err = new LigmaError('exceeded 512 tokens', 'PROVIDER_ERROR');
    expect(classifyError(err)).toMatchObject({ retry: false, reason: 'exceeded 512 tokens' });
  });
  it('reads the structurally carried status on a LigmaError', () => {
    const err = new LigmaError('upstream is unhappy', 'PROVIDER_ERROR', { status: 503 });
    expect(classifyError(err)).toMatchObject({ retry: true, reason: 'server error (503)' });
  });
});
