...
export class CodexTokenStore {
  ...
  async getValidAccessToken(): Promise<string> {
    ...
    if (this.cache === null) {
      throw new Error(
        'ChatGPT subscription is not signed in or has been signed out. Please sign in again.',
      );
    }
  ...
  private async doRefresh(): Promise<string> {
    ...
    try {
      ...
      // Structured, not scraped: the token endpoint's own status + RFC 6749
      // error code decide whether the refresh token is dead. Anything else
      // (5xx, network) is transient and must propagate, not sign the user out.
      ...
      if (isBadCredential) {
        await this.clear();
        throw new Error('ChatGPT subscription expired. Please sign in again.', { cause: err });
      }
      throw err;
    }
