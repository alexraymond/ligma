...
import { CodexOAuthError, type TokenSet } from './oauth';
...
describe('CodexTokenStore', () => {
  ...
  it('clears stored auth and throws when refresh hits invalid_grant', async () => {
    const refreshFn = vi
      .fn()
      .mockRejectedValue(
        new CodexOAuthError(
          'Codex OAuth refresh failed: 400 {"error":"invalid_grant"}',
          400,
          'invalid_grant',
        ),
      );
    ...
    await expect(store.getValidAccessToken()).rejects.toThrow(/ChatGPT subscription expired/);
    expect(await store.read()).toBeNull();
    await expect(readFile(filePath, 'utf8')).rejects.toMatchObject({ code: 'ENOENT' });
