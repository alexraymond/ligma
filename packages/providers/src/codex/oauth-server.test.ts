...
describe('startCallbackServer', () => {
  ...
  // P15 — a callback that does not carry our state is not ours. Answer it 400
  // and leave the pending login alone; a stray local request (prefetch, stale
  // tab, a second app on the port) must not kill a live sign-in.
  it('answers a state-mismatched callback 400 without settling the pending wait', async () => {
    const server = await track(await startCallbackServer(0));
    const waiter = server.waitForCode('foo');

    const stray = await fetch(`${server.redirectUri}?code=X&state=bar`);
    expect(stray.status).toBe(400);
    const noState = await fetch(`${server.redirectUri}`);
    expect(noState.status).toBe(400);

    // Still waiting: the real callback lands afterwards and wins.
    const real = await fetch(`${server.redirectUri}?code=GOOD&state=foo`);
    expect(real.status).toBe(200);
    await expect(waiter).resolves.toEqual({ code: 'GOOD', state: 'foo' });
  });
  ...
  it('throws an actionable error when the preferred port is occupied', async () => {
    const first = await track(await startCallbackServer(0));
    const busyPort = portOf(first.redirectUri);
    await expect(startCallbackServer(busyPort)).rejects.toThrow(/already in use/);
  });
});
