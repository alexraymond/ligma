...
export async function startCallbackServer(preferredPort?: number): Promise<CallbackServer> {
  ...
  try {
    ...
    if (code === 'EADDRINUSE') {
      throw new Error(
        `Codex OAuth callback port ${firstPort} is already in use (usually another ligma or Codex CLI instance). Close them and retry.`,
      );
    }
    throw err;
  }
  ...
  const handleCallback = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    // A request that does not carry OUR state is not our callback: a browser
    // prefetch, a stale tab, another app on the port. Answer it, but never let
    // it settle the pending login — killing a live sign-in on an unattributable
    // request is how "nothing happened when I clicked authorize" happens.
    if (pending && state !== pending.expectedState) {
