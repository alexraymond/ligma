...
/**
 * Token-endpoint failure with the two facts callers actually branch on carried
 * structurally: the HTTP status and the RFC 6749 `error` code from the body.
 * The token store used to decide "credential is dead, sign in again" by
 * regexing `/\b400\b/` over this message — which also fires on a 500 whose
 * body happens to mention 400, and misses a 400 whose text does not.
 */
...
async function postToken(
  ...
  if (!res.ok) {
    ...
    throw new CodexOAuthError(
      `Codex OAuth ${kind} failed: ${res.status} ${text}`,
      res.status,
      oauthErrorCode(text),
    );
  }
