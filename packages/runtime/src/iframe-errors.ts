...
export function isIframeErrorMessage(data: unknown): data is IframeErrorMessage {
  return (
    typeof data === 'object' &&
    data !== null &&
    // Accept the pre-rebrand `__codesign` tag too — a design generated
    // before the rename embeds a frozen overlay bundle that still emits it.
    ((data as { __ligma?: boolean }).__ligma === true ||
