...
export const OVERLAY_SCRIPT = `(function() {
  ...
  function onParentMessage(ev) {
    ...
    var data = ev.data;
    if (!data || (data.__ligma !== true && data.__codesign !== true)) return;
...
/**
 * Envelope tag accepted from both the current overlay (`__ligma`) and a
 * pre-rebrand overlay bundle (`__codesign`) — a design generated before the
 * rename embeds a frozen copy of `OVERLAY_SCRIPT` in its stored HTML and
 * will keep emitting the old tag forever. Accepting both keeps that already
 * persisted output working; new designs pick up `__ligma` from the sender
 * changes above.
 */
function hasLigmaEnvelope(data: object): boolean {
  const d = data as { __ligma?: unknown; __codesign?: unknown };
  return d.__ligma === true || d.__codesign === true;
}
...
export function isElementRectsMessage(data: unknown): data is ElementRectsMessage {
  ...
  const d = data as { type?: string; entries?: unknown };
  if (!hasLigmaEnvelope(data) || d.type !== 'ELEMENT_RECTS') return false;
...
export function isCanvasSizeMessage(data: unknown): data is CanvasSizeMessage {
  if (typeof data !== 'object' || data === null) return false;
  const d = data as { type?: string; width?: unknown; height?: unknown };
  return (
    hasLigmaEnvelope(data) &&
    d.type === 'CANVAS_SIZE' &&
...
export function isCanvasPanDragMessage(data: unknown): data is CanvasPanDragMessage {
  if (typeof data !== 'object' || data === null) return false;
  const d = data as { type?: string; dx?: unknown; dy?: unknown };
  return (
    hasLigmaEnvelope(data) &&
...
export function isArtboardSelectedMessage(data: unknown): data is ArtboardSelectedMessage {
  ...
  if (!hasLigmaEnvelope(data) || d.type !== 'ARTBOARD_SELECTED') return false;
