...
describe('OVERLAY_SCRIPT reattach loop warning throttle', () => {
  it('dedupes repeated reattach failures across many ticks', () => {
    ...
    // 9 unique install event types (mouseover/mouseout/click/pointerdown/
    // pointermove/pointerup/pointercancel/wheel/submit) * 2 ops (remove+add)
    // = 18 distinct warn keys at most. The invariant tested here is that the
    // warn count doesn't scale with tick.
    expect(warn.mock.calls.length).toBeLessThanOrEqual(18);
  });
  ...
  it('emits at most one warn per unique error key over the whole loop', () => {
    ...
    // should be ≤ 9 (one per unique install event type), well under the
    // 25-tick spam ceiling.
...
describe('OVERLAY_SCRIPT SET_MODE source validation', () => {
  it('drops SET_MODE messages whose source is not window.parent (forged)', () => {
    ...
    onMessage?.({
      source: forgedSource,
      data: { __ligma: true, type: 'SET_MODE', mode: 'comment' },
    });
  ...
  it('also accepts the pre-rebrand __codesign envelope tag (legacy host sends it)', () => {
    const h = runOverlayWithHarness();
    const onMessage = h.windowListeners.get('message');
    const onClick = h.documentListeners.get('click');
...
describe('OVERLAY_SCRIPT rect broadcast', () => {
  it('broadcasts ELEMENT_RECTS after WATCH_SELECTORS message', () => {
    ...
    onMessage?.({
      source: h.parent,
      data: { __ligma: true, type: 'WATCH_SELECTORS', selectors: ['#a'] },
    });
    h.runRaf();
