 ...
 *
 * Handles any number of sequential artifacts in one stream (state is reset at
 * every `</artifact>`), but NOT nested tags: an `<artifact>` inside an
 * `<artifact>` is read as content of the outer one. Identifier collisions and
 * `type` validation are the caller's problem — the parser reports what the
 * model wrote and does not judge it.
 */
