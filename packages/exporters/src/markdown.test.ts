...
describe('sanitizeUrl encoded-scheme bypass guard', () => {
  ...
  // P5 — named refs the decoder never knew about (`&colon;`, `&NewLine;`,
  // `&Tab;`) reconstruct into a real scheme inside a CommonMark renderer.
  it('rejects named-entity encoded javascript schemes', () => {
    expect(sanitizeUrl('javascript&colon;alert(1)', 'link')).toBeNull();
    expect(sanitizeUrl('java&Tab;script:alert(1)', 'link')).toBeNull();
    expect(sanitizeUrl('&NewLine;javascript:alert(1)', 'link')).toBeNull();
    expect(sanitizeUrl('data&colon;text/html,<script>x</script>', 'image')).toBeNull();
  });
