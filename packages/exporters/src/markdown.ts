...
export function sanitizeUrl(raw: string, kind: 'link' | 'image'): string | null {
  ...
  // Any surviving entity reference in the scheme segment is a bypass attempt:
  // `javascript&colon;alert(1)` carries no literal ':' so the scheme test below
  // never fires, yet a CommonMark renderer reconstructs `javascript:`.
  // `decodeEntities` only knows a handful of named refs, so rather than chase
  // the full HTML5 table we reject the shape outright.
  if (/^[^:/?#]*&[a-z0-9#]+;/i.test(probe)) return null;
