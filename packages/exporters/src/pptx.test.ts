...
describe('extractSlides', () => {
  it('walks the declared .slide sections when the artifact is a deck', () => {
    const html = `
      <div class="deck">
        <section class="slide" data-title="Cover"><h1>One</h1><p>alpha</p>
          <aside class="notes">say this out loud</aside></section>
        <section class="slide"><h2>Two</h2><p>beta</p></section>
      </div>
      <footer><section class="deck-footer">chrome, not a slide</section></footer>
    `;
    const slides = extractSlides(html);
    expect(slides).toHaveLength(2);
    expect(slides[0]).toEqual({ title: 'One', bullets: ['alpha'], notes: 'say this out loud' });
    expect(slides[1]).toEqual({ title: 'Two', bullets: ['beta'], notes: '' });
  });
  ...
  it('treats each <section> as a slide and pulls the heading + bullets', () => {
    ...
    const slides = extractSlides(html);
    expect(slides).toHaveLength(2);
    expect(slides[0]).toEqual({ title: 'One', bullets: ['alpha', 'beta'], notes: '' });
    expect(slides[1]).toEqual({ title: 'Two', bullets: ['paragraph body'], notes: '' });
  });

  it('falls back to a single slide when no <section> exists', () => {
    const slides = extractSlides('<h1>Solo</h1><p>body</p>');
    expect(slides).toEqual([{ title: 'Solo', bullets: ['body'], notes: '' }]);
  });

  it('preserves CJK characters end-to-end', () => {
    const slides = extractSlides('<section><h1>你好</h1><p>世界</p></section>');
    expect(slides[0]).toEqual({ title: '你好', bullets: ['世界'], notes: '' });
  });
...
describe('exportPptx', () => {
  ...
  it('writes one slide per deck slide for a vendored deck template', async () => {
    const { readFileSync } = await import('node:fs');
    const html = readFileSync(
      join(import.meta.dirname, '../../../design-templates/html-ppt-pitch-deck/example.html'),
      'utf8',
    );
    expect(extractSlides(html)).toHaveLength(10);

    const dest = join(tempDir, 'deck.pptx');
    await exportPptx(html, dest, { deckTitle: 'Pitch' });
    const { readFile } = await import('node:fs/promises');
    const buf = await readFile(dest);
    // Each slide is its own `ppt/slides/slideN.xml` entry in the zip.
    const names = buf.toString('latin1').match(/ppt\/slides\/slide\d+\.xml(?!\.rels)/g) ?? [];
