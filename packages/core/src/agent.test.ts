...
describe('generateViaAgent() — Phase 1 pass-through', () => {
  it('throws LigmaError on empty prompt (matches generate())', async () => {
    await expect(
  ...
  it('rethrows the original input.getApiKey error (preserves structured code)', async () => {
    ...
    // LigmaError(PROVIDER_AUTH_MISSING). Without the capture-and-rethrow
    ...
    const authErr = new LigmaError('ChatGPT 订阅已失效', ERROR_CODES.PROVIDER_AUTH_MISSING);
    await expect(
      generateViaAgent({
