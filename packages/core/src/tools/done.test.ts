...
function App() { return <div>Hi</div>; }`,
  ...
  // P27 — markup inside a JS string/template/comment is not markup. Reporting
  // it sent the agent into a fix loop over an error that never existed.
  it('does not report tags or ids that only appear inside JS literals', async () => {
    const fs = makeFs({
      'index.html': `const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{}/*EDITMODE-END*/;
const snippet = "<div><span>unclosed in a string";
const tpl = \`<img src="x">\`;
// <section id="dupe"> and <section id="dupe"> in a comment
function App() { return <div id="root-el">Hi</div>; }
ReactDOM.createRoot(document.getElementById("root")).render(<App/>);`,
    });
    const tool = makeDoneTool(fs);
    const res = await tool.execute('id-masked', {});
    expect(res.details.errors).toEqual([]);
    expect(res.details.status).toBe('ok');
  });

  it('still reports real unclosed tags in plain HTML', async () => {
    const fs = makeFs({ 'index.html': '<html><body><div>oops</body></html>' });
    const tool = makeDoneTool(fs);
    const res = await tool.execute('id-plain-html', {});
    expect(res.details.errors.some((e) => /Unclosed <div>/.test(e.message))).toBe(true);
  });
});
