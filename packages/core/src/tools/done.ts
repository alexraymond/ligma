...
/**
 * Blank out every string literal, template literal and comment, replacing
 * their characters with spaces (newlines kept, so line numbers still line up).
 *
 * P27: the HTML lint below scans with regexes that know nothing about JS, so
 * `const markup = "<div>"` inside a JSX artifact reported a phantom "Unclosed
 * <div>". The agent then "fixed" it, `done` reported it again, and the loop
 * burned its five rounds on an error that was never there.
 */
function maskLiteralsAndComments(src: string): string {
  const out: string[] = [];
  let inStr: '"' | "'" | '`' | null = null;
  let escaped = false;
  ...
  for (let i = 0; i < src.length; i += 1) {
    ...
    if (inLineComment) {
      // `push` keeps newlines verbatim, so blanking unconditionally is safe
      // and the line count stays identical to the original source.
      if (ch === '\n') inLineComment = false;
      push(ch, true);
      continue;
    }
...
function findJsxStructuralIssues(src: string, masked: string): DoneError[] {
  if (!looksLikeJsx(src)) return [];
  ...
  // Brace / paren / bracket balance across the whole file, over the masked
  // source so JSX string literals and template literals don't confuse it.
  const counters = { '(': 0, '{': 0, '[': 0 };
  for (const ch of masked) {
    if (ch === '(' || ch === '{' || ch === '[') counters[ch] += 1;
...
export function makeDoneTool(
  ...
  return {
    ...
    async execute(_id, params): Promise<AgentToolResult<DoneDetails>> {
      ...
      // The HTML lint below is regex-only and knows nothing about JS, so for a
      // JSX artifact it runs over a copy with string/template/comment spans
      // blanked out — same length, same line numbers, no markup inside
      // literals (P27). Pure HTML is scanned as-is.
      const scanned = looksLikeJsx(file.content)
        ? maskLiteralsAndComments(file.content)
        : file.content;
      const errors: DoneError[] = [
        ...findJsxStructuralIssues(file.content, scanned),
        ...findUnclosedTags(scanned),
        ...findDuplicateIds(scanned),
        ...findMissingAlt(scanned),
      ];
      if (runtimeVerify) {
        try {
