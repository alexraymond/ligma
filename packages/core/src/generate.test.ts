...
import { LigmaError, STORED_DESIGN_SYSTEM_SCHEMA_VERSION } from '@ligma/shared';
...
describe('generate()', () => {
  it('throws LigmaError on empty prompt', async () => {
    await expect(
      generate({ prompt: '   ', history: [], model: MODEL, apiKey: 'sk-test' }),
    ).rejects.toBeInstanceOf(LigmaError);
...
\`\`\``;
  ...
  it('throws LigmaError INPUT_UNSUPPORTED_MODE when mode is not create', async () => {
    await expect(
...
describe('composeSystemPrompt()', () => {
  ...
  it('create mode advertises the device-frames starter assets without hardcoding chrome', () => {
    ...
    // P13 — the filenames must match what FRAME_TEMPLATES actually seeds into
    // the virtual fs (.jsx), or the model's first `view` 404s.
    expect(prompt).toContain('frames/iphone.jsx');
    expect(prompt).toContain('frames/ipad.jsx');
    expect(prompt).toContain('frames/watch.jsx');
    expect(prompt).not.toContain('frames/iphone.html');
  });

  it('names every frame template the virtual fs actually seeds', () => {
    const prompt = composeSystemPrompt({ mode: 'create' });
    for (const [name] of FRAME_TEMPLATES) {
      expect(prompt).toContain(`frames/${name}`);
    }
  });
...
describe('composeSystemPrompt() — progressive disclosure', () => {
  ...
  // P14 — the keyword tables are English + Chinese only. A brief in any other
  // language matched nothing and silently shipped WITHOUT the chart contract.
  it('unmatched brief keeps the full craft + chart guidance', () => {
    for (const userPrompt of [
      'un tableau de bord des ventes trimestrielles',
      'ein Diagramm der Quartalszahlen',
      'algo bonito',
    ]) {
      const p = composeSystemPrompt({ mode: 'create', userPrompt });
      expect(p, `chart contract missing for "${userPrompt}"`).toContain('Chart rendering contract');
      expect(p, `craft directives missing for "${userPrompt}"`).toContain('Craft directives');
    }
  });
