...
The host seeds the agent's virtual filesystem with `frames/iphone.html`,
`frames/ipad.html`, etc. The system prompt mentions only that these
frames exist; the model decides whether the design benefits from device
chrome and `view`s the relevant frame on its own. There is no keyword
detection in either the host or the prompt.
