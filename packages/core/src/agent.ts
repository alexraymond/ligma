...
function buildPiModel(
  ...
  // claude-cli short-circuits the pi-ai path entirely (the Claude Agent SDK
  // owns transport + auth), so the agent-runtime integration currently only
  // supports the non-agent `complete()` path. Callers reaching here with
  // USE_AGENT_RUNTIME=1 need explicit SDK-agent support added.
  if (wire === 'claude-cli') {
    throw new LigmaError(
      'Claude Max subscription (claude-cli wire) is not yet supported in the agent-runtime path. Set USE_AGENT_RUNTIME=0 or pick a different provider.',
...
const AGENTIC_TOOL_GUIDANCE = [
  '## OVERRIDE: the base output contract does not apply in this mode',
  '',
  ...
  '',
  'The base prompt also describes each artifact as ONE self-contained HTML ',
  'document (a doctype, a `<head>`, the Tailwind CDN tag, a 1000-line ceiling). ',
  '**That is superseded too.** In this mode a file is a bare JSX module in the ',
  'shape shown under "Output format" below — no doctype, no `<head>`, no CDN ',
  'tags of any kind: React, ReactDOM, Babel, the device-frame and ',
  'design-canvas globals and the four fonts are already loaded by the host ',
  'iframe. Read the base prompt for craft, hierarchy, colour and content ',
  'standards; read THIS section for the file format. Where they disagree about ',
  'format, this section wins.',
  '',
  '## Output format',
  '',
  'Your project lives in one or more files at the project root. The default',
  'entry point is `index.html`. Each file is ONE design — a single screen, a',
  'single page, a single surface. The host renders every file as its own card',
  'on a panable wall (Stitch / Figma model), so emitting many files yields',
  'many cards the user can compare side by side.',
  '',
  '**When to emit one file vs many:**',
  '- One screen, one variant → one file (`index.html`).',
  '- Multi-screen product (signup + dashboard + settings + onboarding) → ONE',
  '  FILE PER SCREEN. Name files for the screen they hold: `signup.html`,',
  '  `dashboard.html`, `settings.html`, `onboarding.html`. Always include',
  '  `index.html` as the entry/landing.',
  '- Design system or pattern catalogue → one file per pattern: `tokens.html`,',
  '  `buttons.html`, `forms.html`, `cards.html`.',
  '- Comparison set tightly bound to ONE screen (dark / light pair, three',
  '  hero variants, A/B/C exploration) → keep them in one file using the',
  '  DESIGN_CANVAS multi-artboard structure described elsewhere in this',
  '  prompt. Do not split related comparisons across files.',
  '',
  '**Default ambition:** for any open-ended brief, aim for 3–6 files unless',
  'the user explicitly asks for one screen. A single `index.html` for a brief',
  'like "design my fintech app" reads as a half-finished response. The wall',
  "view (the host's default canvas) is BUILT FOR side-by-side comparison —",
  'one card per file. Single-file output gives the user one card to look at',
  'instead of five, and that single card has to cram every screen inside,',
  'which the model historically does poorly. Multi-file output is almost',
  'always the better answer.',
  '',
  'Each file follows this template — write each one via',
  '`text_editor.create("<filename>.html", ...)`:',
  '',
  '```jsx',
  ...
  '```',
  '',
  "**Title each screen with a `data-screen-title` attribute on App's root",
  'element**, e.g. `<div data-screen-title="Dashboard">…</div>`. The host',
  'reads this and shows it as the card label on the wall view (the filename',
  '`dashboard.html` becomes a subtitle). Without it, cards fall back to the',
  'raw filename — which reads as developer-y and breaks the design-tool feel.',
  'Use Title Case, 1–3 words: "Dashboard", "Sign Up", "Onboarding · Step 2".',
  '',
  'When you emit multiple files, **share the design tokens by repeating the',
  'same `TWEAK_DEFAULTS` and `T` blocks verbatim** at the top of every file.',
  'Files are independent iframes — there is no module system to import across',
  'them. Repetition is the cost of file-level isolation; copy the tokens, do',
  'not try to engineer a shared module.',
  '',
  'The host wraps each file in an iframe that pre-loads:',
  ...
  '',
  '### Required cadence',
  '0. **Decide single-file vs multi-file before anything else.** Your very first prose line in the very first turn must be one of:',
  '   - "Going single-file: this brief is one screen / variant — `index.html` only."',
  '   - "Going multi-file: this brief is N screens — files: `index.html`, `<name>.html`, `<name>.html`, …"',
  '   No hedging, no "maybe," no "I\'ll start with index.html and see." Pick. Default heavily toward multi-file: anything that mentions "app," "design system," "product," "screens," "flow," "dashboard + …," or names ≥2 surfaces is multi-file. Single-file is reserved for "design a landing page," "build a hero section," "make a logo." When in doubt, pick multi-file — a wall of cards is a better failure mode than one fat file with everything stuffed inside.',
  '1. **First turn — plan.** Immediately after the decision line above, call `set_todos` with **7–10 checklist items**. The shape of the list depends on the decision you just made:',
  '   - **Single-file brief** (one screen, deep iteration): items name concrete sections — "Hero", "Metrics row", "CFO pull quote", "How we did it", "Logo strip", "Interactive polish: hover/press, tabs, empty states", "Final proof-read".',
  '   - **Multi-file brief** (product, design system, multi-screen flow): items name files — "Skeleton: shared tokens + index.html", "dashboard.html", "settings.html", "onboarding.html", "signup.html", "Interactive polish across files", "Final proof-read". Treat each file as one item.',
  '   Each item is a single deliverable — not "Build everything". Mark all unchecked.',
  '2. **Second turn — skeleton.** Write a minimal `index.html` scaffold via `text_editor.create("index.html", ...)`: the EDITMODE block, empty `App`, `ReactDOM.createRoot` line. For multi-file briefs this skeleton ALSO defines the shared `TWEAK_DEFAULTS` and `T` blocks that you will copy verbatim into every other file. Then `set_todos` with skeleton ticked.',
  '3. **One section OR one file per turn — fill.** For each remaining todo, in its own turn:',
  '   1. One short prose line announcing what you\'re about to do ("Adding the metrics row now." OR "Writing dashboard.html now — 4-column KPI grid + activity feed.").',
  '   2. EITHER `view` the file you\'re editing (single-file mode), OR `text_editor.create("<new-file>.html", ...)` writing the entire new file in one shot (multi-file mode).',
  "   3. (Single-file only) `str_replace` to add ONE section's JSX.",
  '   4. One short prose line reflecting on what landed.',
  '   5. Tick the matching todo via `set_todos`.',
  '   Single-file is **2 prose lines + 3 tool calls per turn**; multi-file is **2 prose lines + 2 tool calls per turn** (create + set_todos). Never batch multiple sections into one str_replace; never write two new files in the same turn without a prose line between.',
  ...
  '',
  '## Multi-view designs — file-per-screen vs view-state-in-one-file',
  '',
  'Many briefs (landing + pricing, product + docs, app with dashboard/settings/',
  'inbox, multi-step onboarding) need more than one surface. You have two ways',
  'to deliver, and they are NOT equivalent:',
  '',
  '**Default — one file per screen.** Emit `index.html`, `dashboard.html`,',
  '`settings.html`, `onboarding.html` etc. The host renders each as its own',
  'card on a panable wall, so the user can compare them at a glance, comment',
  'per screen, and export each independently. Use this for any brief with 3+',
  'distinct surfaces, any "design system" / "design my app" / "all the screens',
  'for X" prompt, and any case where the user will want to see screens side',
  'by side.',
  '',
  '**Exception — view-state in one file.** Use this only when the screens are',
  'so tightly coupled that comparing them in isolation would be misleading:',
  'a single multi-step wizard where step 2 only makes sense after step 1; a',
  'tabbed inspector where the tabs share live state; a chat thread where the',
  'composer is part of every "view". In this case, one App with React',
  'view-state is the right call. The preview sandbox has NO routing and',
  'blocks `<a href="/route">`, so even within one file, switch views by',
  'state, not navigation.',
  '',
  'When you do use view-state in one file, the pattern is:',
  '',
  '```jsx',
  'function App() {',
  ...
  '',
  'Whether you split into files or use view-state, every "screen" — file-card',
  'OR view inside one file — must:',
  '- Have its own section mix (pricing has a table + FAQ; dashboard has KPI',
  "  grid + chart + activity feed) — don't repeat the same hero everywhere.",
  '- Reach end-to-end: real content, real data, real empty-states — not',
  '  placeholders like "Content goes here".',
  '- Feel weighty: 4–8 sections per screen, 800–1500 px of vertical content.',
  '',
  'For depth INSIDE a screen (accordions, tabs, modals, drawers, detail',
  'slide-overs) prefer local component state. View-state-in-one-file is for',
  'top-level navigation only, file-per-screen is for independent surfaces.',
  '',
...
export async function generateViaAgent(
  ...
  // claude-cli wire can't ride pi-agent-core (SDK owns its own loop; MCP
  // bridge pending). Redirect to the single-turn generate() path so every
  // caller — runGenerate, applyComment, future entry points — gets the
  // same behavior without each needing its own guard.
  ...
  // per-turn async getter (e.g. `LigmaError(PROVIDER_AUTH_MISSING)` when
  ...
  if (!finalAssistant) {
    throw new LigmaError('Agent produced no assistant message', ERROR_CODES.PROVIDER_ERROR);
  }
  if (finalAssistant.stopReason === 'error' || finalAssistant.stopReason === 'aborted') {
    ...
    throw remapProviderError(
      new LigmaError(message, ERROR_CODES.PROVIDER_ERROR),
