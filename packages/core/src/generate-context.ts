/**
 * Request-context composition shared by every generation entry point.
 *
 * `generate()`, `applyComment()` and `generateViaNewLoop()` all have to turn
 * the same raw inputs — a design system, local attachments, a reference URL,
 * the builtin skill catalogue — into prompt text and provider image inputs.
 * This module is that single implementation. It used to live private inside
 * `./index.ts`, which is how the new-loop path came to ship without any of it
 * (P4): the helpers simply weren't reachable from there.
 *
 * Pure except for `collectAllSkillBlobs`, which reads the builtin skill
 * directory best-effort and reports failures as warnings rather than throwing.
 */

import { filterActive, formatSkillsForPrompt } from '@ligma/providers';
import type { LoadedSkill, StoredDesignSystem, WireApi } from '@ligma/shared';
...
export function imageInputsForWire(
...
export function buildPrompt(prompt: string, contextSections: string[]): string {
...
export async function collectAllSkillBlobs(
  log: CoreLogger,
  providerId: string,
