...
When the design calls for a specific device — phone, tablet, watch, desktop
browser — a set of JSX starter modules with accurate device chrome (rounded
frame, status bar, dynamic island, home indicator, digital crown, browser
chrome) is seeded under \`frames/\` in the virtual filesystem:

  frames/iphone.jsx
  frames/ipad.jsx
  frames/watch.jsx
  frames/android.jsx
  frames/macos-safari.jsx

They are \`.jsx\`, not \`.html\` — the same module shape every artifact uses
(EDITMODE block, \`function App()\`, \`ReactDOM.createRoot(...)\`), built against
the runtime's pre-loaded \`IOSDevice\` / \`DesignCanvas\` globals.

If you decide the design benefits from device chrome, \`view\` the relevant
frame first, then replace the screen contents inside the \`<IOSDevice>\` (or
equivalent) wrapper — keeping the chrome and the EDITMODE block untouched.
Otherwise ignore them and write a freeform layout. The choice is yours;
nothing forces a frame.`;
...
**Artboards must be direct children of their section.** When you have a single artboard in a section, do NOT wrap it in any grouping element (\`<div>\`, \`<data-artboard-row>\`, etc.). The single-artboard chrome-suppression CSS uses \`:has(> [data-artboard]:only-child)\` which only fires on direct children — wrapping breaks it and leaves stray section/artboard chrome around what should look like a plain single-screen design.
...
/* Single-artboard suppression: if a section holds one artboard, hide the
   ...
   single-screen design. The \`> [data-artboard]:only-child\` selector relies
   on the artboard being a *direct* child of the section. Do NOT wrap a
   single artboard in \`[data-artboard-row]\` or any other grouping element —
   place the artboard as a direct child so this rule fires. */
...
// P14 — these tables are English + Chinese only. They may only ever ADD a
// section: a French "tableau de bord" or a German "Diagramm" must not silently
// lose the chart contract, so the no-match path below falls back to the full
// set rather than the thin one. Withholding guidance on a language we happen
// not to list is the failure mode; spending a few extra tokens is not.
// ponytail: keyword table kept — the alternative is an extra model round trip
// to classify the brief before composing its own prompt. Revisit if the
// always-on fallback measurably crushes small-context models.
...
function composeCreateProgressive(userPrompt: string): string[] {
  ...
  if (noMatch) {
    // Unclassifiable brief (or a language the tables above don't cover):
    // ship the full craft + chart guidance rather than a silently thinner
    // prompt. See the note on KEYWORDS_DASHBOARD.
    sections.push(CRAFT_DIRECTIVES);
    sections.push(CHART_RENDERING);
    return sections;
  }
