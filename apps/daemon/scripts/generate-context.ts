...
interface Task {
  ...
  // D7: acceptanceCriteria is deliberately NOT modelled here. Generated agent
  // context must never carry the criteria — part of them is withheld from the
  // builder by the acceptance contract, and ai-context*.md is the first file
  // every agent reads. Leaving the field out makes emitting it a type error.
  tags: string[];
  createdAt: string;
  updatedAt: string;
...
async function main(): Promise<void> {
  ...
  // Build markdown
  const lines: string[] = [];
  lines.push("<!-- generated by generate-context.ts — do not hand-edit -->");
  lines.push("# Ligma — AI Context Snapshot");
  ...
  const awaitingVerification = tasks.filter((t) => t.kanban === "awaiting-verification").length;
  lines.push("## Kanban Pipeline");
  lines.push(`Not Started: ${notStarted} | In Progress: ${inProgressTasks.length} | Awaiting Verification: ${awaitingVerification} | Done: ${doneTasks.length}`);
  lines.push("");
  ...
  await writeFile(path.join(DATA_DIR, "ai-context-readable.md"), content, "utf-8");
  console.log(`Generated ai-context.md + ai-context-readable.md (${content.length} bytes)`);
}
