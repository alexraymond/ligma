 ...
 * baseUrl / apiKey surface — it's handled via ClaudeCliLoginCard instead —
 * so narrow the modal's wire here to exclude it before any canonicalBaseUrl
 * call. An invariant violation surfaces as a thrown error rather than a
 * silent fall-through.
 */
function assertHttpWire(wire: WireApi): Exclude<WireApi, 'claude-cli'> {
  if (wire === 'claude-cli') {
    throw new Error(
      'AddCustomProviderModal does not support the claude-cli wire — use the Claude Max subscription card in Settings.',
    );
  }
  return wire;
}
      ...
      if (isEdit && editTarget !== undefined) {
        ...
        if (!lockEndpoint) {
          if (baseUrl.trim() !== editTarget.baseUrl) {
            update.baseUrl = canonicalBaseUrl(baseUrl.trim(), assertHttpWire(wire));
          }
        ...
        await window.codesign.config.addProvider({
          ...
          name: name.trim() || id,
          wire,
          baseUrl: canonicalBaseUrl(baseUrl.trim(), assertHttpWire(wire)),
          apiKey: apiKey.trim(),
