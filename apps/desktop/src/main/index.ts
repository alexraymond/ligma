...
import { registerClaudeCliIpc } from './claude-cli-ipc';
import {
...
function registerIpcHandlers(db: Database | null): void {
  ...
  const runGenerate = (
    ...
    // this, users would have to set USE_AGENT_RUNTIME=0 just to use the
    // Claude Max subscription provider.
    if (!USE_AGENT_RUNTIME || input.wire === 'claude-cli') return generate(input);
    const sendEvent = (event: AgentStreamEvent) => {
      mainWindow?.webContents.send('agent:event:v1', event);
    };
    const baseCtx = { designId: designId ?? '', generationId: id } as const;
