...
function buildEndpointForWire(
  ...
  if (wire === 'claude-cli') {
    throw new Error(
      'claude-cli wire has no HTTP /models endpoint; runProviderTest must short-circuit before reaching buildEndpointForWire.',
    );
  }
...
async function testClaudeCliSubscription(): Promise<ConnectionTestResponse> {
  ...
  // A present CLI is a strong signal it's also logged in (`claude` itself
  // blocks on an interactive sign-in otherwise). A full SDK ping here
  // would add ~5s + real token spend, which is too costly for a
  // connection test. If the user's session is invalid, the first real
  // generation surfaces a clean error.
  return { ok: true };
}
...
async function runProviderTest(creds: ActiveProviderCredentials): Promise<ConnectionTestResponse> {
  ...
  if (creds.wire === 'claude-cli') {
    return testClaudeCliSubscription();
  }
