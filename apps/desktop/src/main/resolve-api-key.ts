...
export async function resolveActiveApiKey(
  ...
  // Claude Max subscription: auth lives in the Claude Code SDK's own
  // Keychain entry, not in our key store. The providers-package adapter
  // short-circuits the pi-ai path and never reads this value, so return
  // an empty string rather than surfacing a PROVIDER_KEY_MISSING error.
  if (providerId === CLAUDE_CLI_PROVIDER_ID) return '';
  try {
